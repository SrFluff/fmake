...
def readConfig(configName):
    if not os.path.exists(configName):
        print("WARN | No such config available for reading")
        return ("gcc","main.c","main",True,"Compiling done!",False,False,"-j$(nproc)")
    f = open(configName,"r")
    cc = "gcc"
    src = "main.c"
    out = "main"
...
def genConfig(fileName="make.fmake",overWrite=False,gCC="gcc",gSRC="main.c",gOUT="main",gMSG=True,gSAY="Compiling done!",gRUN=False,gMKE=False,gMKF="-j$(nproc)"):
    ...
    if os.path.exists(fileName) and not overWrite:
        print("WARN | overWrite is set to True, skipping overwrite")
        return -1
